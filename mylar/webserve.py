...
class WebInterface(object):
    ...
    def searchit(self, name, issue=None, smode=None, search_type=None, serinfo=None):
        ...
        if search_type == 'comic' and smode == 'pullseries':
            ...
            try:
                ...
                logger.error('[%s] Unable to perform required one-off pull-list search for : [name: %s][issue: %s][mode: %s]' % (e, name, issue, smode))
                return
    ...
    def addComic(self, comicid, comicname=None, comicyear=None, comicimage=None, comicissues=None, comicpublisher=None, imported=None, ogcname=None, serinfo=None):
        ...
        if imported == "confirm":
            ...
            logger.fdebug('serinfo:' + str(serinfo))
        ...
        for cid in comicidlist:
            ...
            if issuelist:
                for issue in issuelist:
                    ...
                    if renameiss is None:
                        logger.error('Failed to preview rename for %s - skipping.' % issue['Location'])
                        continue
                    ...
                    if filename.lower().endswith(extensions):
                        ...
                        for issue in issues:
                            if issue['Location'] == filename:
                                ...
                                if renameiss is None:
                                    logger.error('Failed to rename %s - skipping.' % filename)
                                    continue
    ...
    def forceRescan(self, ComicID, bulk=False, action='recheck', api=False):
        if bulk:
            ...
            if action == 'recheck':
                ...
                mylar.GLOBAL_MESSAGES = {'status': 'success', 'comicid': None, 'tables': 'both', 'message': 'Finished complete series (re)tagging of %s series' % (str(len(ComicID)))}
        else:
            myDB = db.DBConnection()
           ...
           #                    storyarcbanner += 'H' + str(bannerheight)
            ...
            if filepath is not None:
                ...
                #    storyarcbanner += 'H' + str(bannerheight)
                    ...
                    from lib import get_image_size
    ...
    def importReadlist(self, filename):
        ...
        if Arc_MultipleSeries is None:
            ...
            for Arc_MS in Arc_MultipleSeries:
                logger.fdebug('Arc_MS: %s' % str(Arc_MS))
    ...
    def preSearchit(self, ComicName, comiclist=None, mimp=0, volume=None, displaycomic=None, comicid=None, dynamicname=None, displayline=None):
        ...
        with importlock:
            ...
            try:
                ...
                # -- SECTION 1: DIRECT COMICID MATCHING -- #
                RemoveIDS = []
                for comicinfo in comiclist:
                    try:
                        logger.fdebug(f"[IMPORT] Checking for any valid ComicID's already present within filenames: {comicinfo}")
                        if not comicinfo['ComicID'] or comicinfo['ComicID'] == 'None':
                            continue
                        ...
                        skipped_items.append(comicinfo.get('ComicName', 'Unknown'))
                        continue
                ...
                # -- SECTION 2: SEARCH-BASED IMPORT PROCESSING -- #
                for cl in comiclist:
                    try:
                        logger.log_memory(f"[IMPORT-PRESEARCH] Processing {cl.get('ComicName', 'Unknown')}")
                        ComicName = cl['ComicName']
                        volume = cl['Volume']
                        ...
                        for result in results:
                            ...
                            if watchmatched.startswith('C'):
                                ...
                                if not result.get('ComicLocation'):
                                    logger.fdebug('[IMPORT] ComicLocation is None for result %s. Skipping.' % result.get('impID', 'Unknown'))
                                    continue
                        ...
                        logger.fdebug(traceback.format_exc())
                        skipped_items.append(cl.get('ComicName', 'Unknown'))
                        continue
                ...
                mylar.IMPORTLOCK = False
                if skipped_items:
                    logger.warn(f"[IMPORT] Import completed with {len(skipped_items)} items skipped: {skipped_items}")
                    
                    # Focused Notification Dispatcher
                    try:
                        from mylar import notifiers
                        message = f"The mass import finished but skipped {len(skipped_items)} items: {', '.join(skipped_items)}"
                        title = "Mylar Import Complete (with skips)"

                        if mylar.CONFIG.PROWL_ENABLED:
                            notifiers.PROWL().notify(message, title)
                        if mylar.CONFIG.PUSHOVER_ENABLED:
                            notifiers.PUSHOVER().notify(title, message=message)
                        if mylar.CONFIG.TELEGRAM_ENABLED:
                            notifiers.TELEGRAM().notify(f"{title}: {message}")
                        if mylar.CONFIG.DISCORD_ENABLED:
                            notifiers.DISCORD().notify(title, message)
                        if mylar.CONFIG.SLACK_ENABLED:
                            notifiers.SLACK().notify(title, message)
                        if mylar.CONFIG.EMAIL_ENABLED:
                            notifiers.EMAIL().notify(message, title)
                        if mylar.CONFIG.GOTIFY_ENABLED:
                            notifiers.GOTIFY().notify(title, message)

                    except Exception as ne:
                        logger.fdebug(f"[IMPORT] Failed to send notification: {ne}")
                else:
...
#--- works, just need to redo search.py to accomodate multiple search terms
    ...
    def get_the_hash(self, filepath):
        import hashlib, io
        from lib.rtorrent.lib import bencode
