...
def post_process_folder(folder_path: str, nzb_name: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    ...
    logger.info(f"[Tasks] Triggering post-processing for folder: {folder_path}")
    
    async def _run():
        async with async_session() as session:
            pp_service = PostProcessorService(session)
            return await pp_service.scan_and_process(folder_path, nzb_name)
